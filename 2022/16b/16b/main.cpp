...
int hsize = 0;
void calculate(int vId, int eId, int flowSum, int totalFlow, int remainingTime) {
    ...
    if (remainingTime <= 1 || totalFlow+231*remainingTime<=2790)
        return;
    if (vId>eId)
        swap(vId, eId);

    if (history[(vId*57+eId)*26*65536ll+(remainingTime-1)*65536+openValves]>=totalFlow){
        same++;
        return;
    }
    else
        history[(vId*57+eId)*26*65536ll+(remainingTime-1)*65536+openValves] = totalFlow;

    if (hsize*1000000 < history.size()) {
        hsize++;
        cout << "hsize: " << history.size() << endl;
        cout << "same: " << same << endl;
    }
    ...
    if (v1.flow && !valveOpen(v1.valveId)) {
        ...
        if (v2.flow && !valveOpen(v2.valveId)) {
            openValve(v2.valveId);
            calculate(vId, eId, fsum+v2.flow, totalFlow+flowSum, remainingTime-1);
            closeValve(v2.valveId);
        }
        for (auto nId : v2.neighbours)
            calculate(vId, nId, fsum, totalFlow+flowSum, remainingTime-1);
    ...
    for (auto nId1 : v1.neighbours) {
        if (v2.flow && !valveOpen(v2.valveId)) {
            openValve(v2.valveId);
            calculate(nId1, eId, flowSum+v2.flow, totalFlow+flowSum, remainingTime-1);
            closeValve(v2.valveId);
        }
        for (auto nId2 : v2.neighbours)
            calculate(nId1, nId2, flowSum, totalFlow+flowSum, remainingTime-1);
    }
}
